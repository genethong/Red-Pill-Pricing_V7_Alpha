...
const N = (v: number): XCell => ({ t: 'n', v: Number.isFinite(v) ? v : 0 });
const F = (formula: string, cached = 0): XCell => ({ t: 'f', v: formula, cached });
...
export function buildLifeSimSpreadsheetXml(
  ...
  const readMe: (XCell | null)[][] = [
    [S('Life simulation — how the commercial numbers are built')],
    [],
  ...
  const cf: (XCell | null)[][] = [[
    S('Year'),
    S('Keep_CAPEX'), S('Keep_OPEX'), S('Keep_Fuel'), S('Keep_Outflow'),
    S('A_CAPEX'), S('A_OPEX'), S('A_Fuel'), S('A_Outflow'),
    S('B_CAPEX'), S('B_OPEX'), S('B_Fuel'), S('B_Outflow'),
    S('dA_CAPEX'), S('dA_OPEX_Fuel'), S('dA_Outflow'),
    S('dB_CAPEX'), S('dB_OPEX_Fuel'), S('dB_Outflow'),
    S('dBmA_CAPEX'), S('dBmA_OPEX_Fuel'), S('dBmA_Outflow'),
    S('Keep_SysOut'), S('A_SysOut'), S('B_SysOut')
  ]];
  ...
  const deltas: (XCell | null)[][] = [
    ...
    [],
    [S('Metric'), S('Keep-this-grid'), S('Mode A'), S('Mode B'), S('Δ A vs Keep'), S('Δ B vs Keep'), S('Δ B vs A'), S('How the delta is made')],
    [
